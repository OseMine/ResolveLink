...
export function Screenshots() {
  ...
  return (
    <section id="screenshots" className="py-16 md:py-24 relative z-10">
      <div
        ref={ref}
        className={cn(
          'max-w-[900px] mx-auto px-5 md:px-8 transition-all duration-700',
          visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10',
        )}
      >
        <div className="text-center mb-8 md:mb-10">
          <span className="site-label mb-4 inline-block">Screenshots</span>
          <h2 className="text-[clamp(1.5rem,4vw,2.5rem)] font-bold tracking-tight text-[#e0e0e0]">
            See it in action
          </h2>
        </div>
        ...
        <div className="flex justify-center gap-1.5 sm:gap-2 mb-5 md:mb-6">
          {screenshots.map((s, i) => (
            <button
              ...
              onClick={() => setActive(i)}
              className={cn(
                'px-3 sm:px-4 py-2 rounded-lg text-[0.75rem] sm:text-[0.8rem] font-medium transition-all border whitespace-nowrap',
