...
```
 ...
 2. Select audio clips in dialog
         │
         ▼
 3. Server generates .lua + .json
 4. Server queues job or launches REAPER
         │
         ▼
                   5. reaper-callback.lua picks up job
                   6. REAPER creates project + items
                   7. Audio engineer mixes
                   8. Engineer renders
         │
         ▼
 9. File watcher detects audio render
10. Auto-import to Resolve
11. Compound clip created
```
...
### REAPER Step 3: Server Generates Scripts
...
5. Generates a Lua import script to `temp/{uuid}_import.lua`
6. Stores the link in memory with status **"Created"**

### REAPER Step 4: Auto-Workflow

The server triggers the REAPER auto-workflow:

**If REAPER is running (`reaper-callback.lua` active):**
1. The server queues a `reaper-create` job in the job queue
2. Link status changes to **"Queued"**
3. `reaper-callback.lua` polls `/api/jobs/pending` using `reaper.defer()`
4. The script picks up the job and executes the Lua import script
5. Link status changes to **"Linked"**

**If REAPER is not running:**
1. The server launches REAPER with the `-r` flag pointing to the generated Lua script
2. Link status changes to **"Sending"**
3. REAPER opens and executes the script on startup
4. `reaper-callback.lua` picks up once REAPER is running

### REAPER Step 5: Project Created in REAPER

The Lua script execution creates:
1. A new REAPER project (or appends to existing)
2. Media items for each audio clip, positioned at the correct time on the timeline
3. Items grouped by track (matching Resolve track indices)
...
When mixing is complete, render the project:

**Option A: Render via generated script**
1. Use the render script generated by `GET /api/links/:id/reaper-render-script`
2. Run the script in REAPER (**Actions > Show action list > Browse > Run**)
3. The script opens REAPER's render dialog pre-configured with the correct output path and format

**Option B: Render manually**
1. Go to **File > Render** in REAPER
2. Set the output directory to `exports/`
3. Use a filename that includes the link ID
4. Choose your format (WAV, FLAC, AIFF, etc.)
5. Start the render

### REAPER Step 8: File Watcher Detects Render

The Chokidar file watcher monitors the `exports/` directory. When a new audio file (`.wav`, `.flac`, `.aiff`, `.ogg`, `.mp3`) appears:

1. The file is detected (with 1-second debounce)
2. The server matches it to an active link by filename convention
3. Link status updates to **"Rendered"**
4. A WebSocket message is broadcast to the UI
5. The server triggers the import-back process (Step 9)

### REAPER Step 9: Compound Clip Import

Once an audio render is detected and matched, the server imports it back into DaVinci Resolve:

1. The rendered audio file is imported into the Resolve media pool
2. All original audio clips on the target track are disabled
3. The rendered clip is appended to the timeline
4. A compound clip is created
5. Link status updates to **"Imported"**

### REAPER Step 10: Review in Resolve

The compound clip is now on your Resolve timeline:
- The rendered audio plays on top
- Original audio clips are preserved underneath (disabled)
- You can double-click the compound clip to see both layers
- To revert, disable the compound clip and enable the originals
...
| **Script Format** | `.jsx` (ExtendScript) + `.json` | `.lua` (Lua) + `.json` |
| **Job Type** | `execute-jsx` | `reaper-create` |
| **Polling** | CEP extension (`client.js`) | `reaper-callback.lua` via `reaper.defer()` |
| **Render Formats** | `.mov`, `.mp4` | `.wav`, `.flac`, `.aiff`, `.ogg`, `.mp3` |
| **Frame/Time Basis** | Frames → seconds | Frames → seconds |
| **Auto-Import** | Compound clip (video) | Compound clip (audio) |
...
1. **Save REAPER projects** — Before rendering, save your `.RPP` project
